...
|---|---|---|---|---|---|
| F4-R-01 | tool de leitura retorna estado incompleto ou divergente do REST | alto | reutilizar domínio existente, nunca reimplementar query no MCP | Marketing Ops | `mitigated_production_validated` |
| F4-R-02 | o modelo tenta chamar caminho de mutação direta | alto | manter bloqueio no runtime e expor novas mutações só via plano | Hermes Runtime | `mitigated_automated_and_production_guarded` |
| F4-R-03 | schema do plano cresce demais e vira superfície frágil | alto | actions pequenas, allowlist estrita, limite por plano e testes de contrato; previews inválidos permanecem fail-closed | Marketing Ops/Bridge | `mitigated_production_validated` |
| F4-R-04 | conflito de versão gera overwrite silencioso | alto | leitura prévia obrigatória, `expected_version`, nova confirmação após conflito | Marketing Ops | `mitigated_automated_validated` |
| F4-R-05 | retry duplica campanha, item, versão ou artifact link | alto | idempotência por plano/ação e testes de replay | Marketing Ops | `mitigated_automated_and_production_validated` |
| F4-R-06 | `campaign_note_add` vira overwrite disfarçado de append | médio | contrato append-only e revisão de UX antes da execução | Product/Marketing Ops | `mitigated_production_validated` |
| F4-R-07 | `artifact_link` permita anexar artifact fora do contexto autorizado | alto | usar somente link de artifact existente e owned metadata validada | Marketing Ops | `mitigated_automated_validated` |
...
| F4-R-09 | deep link apontar para rota inconsistente com o frontend real | médio | gerar links no `marketing-ops` com contrato revisado pelo frontend | Frontend | `mitigated_production_validated` |
| F4-R-10 | descrição de tool grande ou ambígua induz uso incorreto pelo modelo | médio | ferramentas pequenas, nomenclatura estável, skill e contrato sistêmico da Bridge revisados; retestes reais e regressões RED/GREEN orientam o wire shape | Hermes Runtime/Bridge | `mitigated_production_validated` |
| F4-R-11 | indisponibilidade do `marketing-ops` aparecer como sucesso conversacional | alto | falha explícita, testes E2E e mensagens seguras de fallback | Bridge/Hermes Runtime | `mitigated_automated_validated` |
...
| F4-R-16 | revisão ENS inventar fato ou não usar fonte oficial | alto | RAG obrigatório, referências mínimas e cenário golden | Hermes Runtime | `mitigated_production_validated` |
| F4-R-17 | falha parcial reexecutar/duplicar ações concluídas | alto | resultado por ação, dependências e replay idempotente | Marketing Ops | `mitigated_automated_validated` |
...
| F4-R-27 | Hermes exibir `deep_link` correto como texto sem navegação clicável | médio | preservar rota server-returned e renderizar Markdown com rótulo por tipo de recurso | Hermes Runtime/Frontend | `mitigated_production_validated` |
...
- seleção aproximada, ambígua ou não exibida de objeto existente;
...
Critério cumprido em 2026-07-29. Os únicos resíduos aceitos são:

- conflito, rate limit e indisponibilidade não foram provocados
  destrutivamente no site público; possuem testes automatizados específicos;
- a primeira chamada inválida de prepare no gate `1.2.4` falhou fechada e foi
  corrigida na mesma conversa, sem plano ou escrita; permanece como telemetria
  de eficiência, não como risco funcional aberto.
