...
| F4-R-03 | schema do plano cresce demais e vira superfície frágil | alto | actions pequenas, allowlist estrita, limite por plano e testes de contrato; preview inválido foi recusado sem persistência e o contrato Bridge separa criação de enriquecimento | Marketing Ops/Bridge | `strict_schema_blocked_invalid_preview_bridge_redeploy_pending` |
...
| F4-R-10 | descrição de tool grande ou ambígua induz uso incorreto pelo modelo | médio | ferramentas pequenas, nomenclatura estável, skill e contrato sistêmico da Bridge revisados; os retestes reais expuseram e corrigiram criação estrita e array `actions` obrigatório por RED/GREEN | Hermes Runtime/Bridge | `fifth_bridge_contract_hotfix_vps_pending` |
