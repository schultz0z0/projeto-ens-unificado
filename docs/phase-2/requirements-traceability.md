...
- **Estado:** `partial_through_task_11`
...
| F2-RF-05 Workspace | agregado e seções sob demanda possuem suporte de domínio | REST e client de detalhe, participantes, materiais e timeline fechados; composição visual é Tasks 12–13 | query keys isolam recursos; integração real/UI pendentes | `backend_and_client_ready_ui_not_started` |
...
| F2-RF-12 Deep links | campanha possui UUID estável e detalhe por ID | lista/criação produzem `/marketing-ops/campaigns/:campaignId`; rota de workspace é Task 12 | navegação unitária/browser coberta; renderização do workspace/E2E pendentes | `deep_link_produced_workspace_route_pending` |
...
| Estados de erro e vazio estão implementados | 9, 11–13 | lista cobre loading/vazio/sem resultado/erro/acesso negado; workspace/painéis pendentes |
| Jornadas críticas são responsivas e acessíveis | 11–14 | lista validada em desktop/mobile e por semântica; workspace, axe e E2E pendentes |
...
| Frontend | unitários, integração, Playwright desktop/mobile e axe | `unit_and_browser_qa_partial_pending_integrated_e2e` |
...
Nenhum requisito está `production_validated`. O fechamento de backend até a Task 9, do client tipado na Task 10 e da lista/criação na Task 11 reduz o trabalho restante, mas não substitui workspace completo, banco real, integrações, deploy ou aceite do piloto.
