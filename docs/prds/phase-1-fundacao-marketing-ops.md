...
O gate local está aprovado e a fundação está `ready_for_production`, permitindo preparar a Fase 2 sem ativar superfícies de usuário. A homologação VPS confirmou probes, CRUD controlado, auditoria/outbox, persistência, concorrência, idempotência, matriz `admin`/`manager`/`member` e isolamento de tenant. A segunda rodada automatizada confirmou o preparo correto da revisão e revelou dois hardenings finais: reconhecimento conservador de confirmações naturais e separação entre rejeições de negócio e falhas de conectividade MCP. As correções passaram no gate local completo e nas imagens Linux. A Fase 1 só fica `completed` após redeploy de `app-bridge`/`hermes-api`, reteste administrativo focado e fechamento em `docs/phase-1/vps-validation.md`.
