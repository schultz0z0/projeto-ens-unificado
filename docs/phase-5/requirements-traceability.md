...
- **Revisão:** 2026-08-06
- **Implementação:** concluída localmente; hotfix aguarda redeploy
...
| F5-RF-04 Decisão | uma decisão humana efetiva | lock, ETag, unique, UI e smoke | `hotfix_ready_awaiting_production` |
...
| F5-RF-10 Notificações | review/status com payload mínimo e deep link | caminho sem `ON CONFLICT`, migration 5.0.5 e probe remoto com rollback | `hotfix_ready_awaiting_production` |
...
| Migration/rollback | 8 migrations remotas aditivas, incluindo forward-fix 5.0.5 | `passed` |
| Gate local | suites, builds, typechecks, audit e security gate | `passed` |
| Gate VPS/navegador | primeira rodada registrada; quatro correções aguardam redeploy | `awaiting_revalidation` |

## Critério de fechamento

As duas divergências bloqueantes e os dois follow-ups foram corrigidos e
regredidos localmente. A matriz só muda para `closed` após novo deploy e
homologação integral descrita em `vps-validation.md`.
