...
## Hardening posterior à segunda rodada de produção

A segunda rodada automatizada confirmou que a revisão administrativa deixou de falhar por tipo e que nenhuma mutação ocorreu antes da confirmação. Duas falhas fechadas ainda impediram a execução: as frases naturais `Confirmo o plano revisado.` e `Confirmo. Execute esse plano novo agora.` não pertenciam à allowlist exata da Bridge, e resultados MCP de erro de negócio eram contabilizados pelo Hermes como falha de conectividade até abrir o circuit breaker de um servidor saudável.

Os testes TDD reproduziram ambos os defeitos antes da implementação. A Bridge agora reconhece somente essas confirmações inequívocas adicionais e continua recusando mensagens condicionais como `Confirmo, mas troque o nome`. O Hermes zera o contador de conectividade quando recebe uma resposta estruturada da tool, mesmo que ela represente rejeição de domínio; somente falhas de transporte, sessão ou conexão incrementam o circuit breaker.

O gate canônico posterior levou 95,5 segundos e retornou código zero: 97 pgTAP, Marketing Ops 51 testes unitários mais 2 E2E, Hermes 13/13, Bridge 69/69, demais suítes, typechecks, builds, audits e Compose aprovados. `app-bridge` e `hermes-api` foram reconstruídos sem cache para Linux; a imagem Hermes aprovou os 4 testes do circuit breaker e as duas imagens continham o código corrigido. O Supabase temporário foi encerrado, as portas oficiais foram restauradas e não houve alteração em `.env`, migrations ou Supabase remoto.

## Binding determinístico após a terceira rodada de produção

A terceira rodada confirmou que a frase `Confirmo o plano revisado.` alcançava `execute_plan` e que o falso circuit breaker não reaparecia, mas a execução recebia `plan_invalid`. Uma sessão isolada reproduziu ainda o Hermes preparando uma revisão após `Sim, mas use outro nome` e tentando executá-la no mesmo turno, embora essa mensagem não possuísse `confirmation_intent`. O backend falhou fechado e nenhuma mutação foi persistida.

A causa raiz estava na fronteira do runtime: a delegação atual já era substituída deterministicamente, mas o `plan_token` continuava sendo copiado pelo modelo a partir de um tool result com envelope MCP e duas camadas JSON. Por TDD, o executor passou a extrair e vincular o token do último `prepare_plan` bem-sucedido em ambos os caminhos de execução. Uma guarda local também decodifica somente para fail-closed a intenção do token atual e bloqueia `execute_plan` no mesmo turno, instruindo o agente a não repetir e a pedir confirmação posterior. O Marketing Ops continua verificando criptograficamente assinatura, ator, tenant, sessão, scopes e JTI.

O gate canônico posterior levou 256 segundos e retornou código zero: 97 pgTAP, Marketing Ops 51 testes unitários mais 2 E2E, Hermes 15/15, Bridge 69/69 e demais suítes, typechecks, builds, audits e Compose aprovados. A imagem Linux `hermes-api` foi reconstruída sem cache e aprovou 19/19 testes combinados do container e circuit breaker. O Supabase temporário foi encerrado, as portas oficiais foram restauradas e não houve alteração em `.env`, migrations ou Supabase remoto.
