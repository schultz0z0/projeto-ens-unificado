...
export function createMarketingOpsMcpServer(deps: MarketingOpsMcpDependencies): McpServer {
  ...
  server.registerTool('marketing_ops_list_campaigns_v1', {
    ...
    inputSchema: { delegation_token: delegationToken, status: z.enum(['draft', 'archived']).optional(), limit: z.number().int().min(1).max(100).default(25) }
  }, async (input) => {
    try {
      if (!deps.features.read) throw new Error('read disabled');
      const actor = await verifyDelegation(input.delegation_token, ['campaign:read'], deps);
      return jsonToolResult({ data: await listCampaigns({ pool: deps.pool, actor, correlationId: actor.correlationId, origin: 'mcp' }, { limit: input.limit, ...(input.status ? { status: input.status } : {}) }) });
  ...
  server.registerTool('marketing_ops_create_campaign_draft_v1', {
    ...
    inputSchema: { delegation_token: delegationToken, idempotency_key: idempotencyKey, name: z.string().min(1).max(200) }
  }, async (input) => {
    try {
      if (!deps.features.write) throw new Error('write disabled');
