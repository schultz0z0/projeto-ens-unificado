...
import { isWorkspacePath, resolveWorkspacePath } from "./workspace-paths.ts";
  ...
  async build(input: {
    ...
    try {
      ...
      for (const artifactId of input.referenceArtifactIds) {
        ...
        if (!isWorkspacePath(entry.relative_path, { prefix: "references/" })) {
          throw new PictureError(
            "picture_reference_path_invalid",
            "A requested reference has an invalid workspace path.",
            400,
          );
        }
        const downloaded = await this.options.artifactClient.downloadArtifact(artifactId, input.signal);
        const targetPath = resolveWorkspacePath(root, entry.relative_path);
